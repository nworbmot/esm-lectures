...
\begin{frame}[fragile]
  \frametitle{Simplified world: linear generation costs, inelastic demand}

  We will now turn to an even simpler world: all the generator cost
  functions are linear
  \begin{align*}
    C_s(g_s) = o_s g_s
  \end{align*}
  and each generator has limited output $0 \leq g_s \leq G_s$.


  We also fix the demand to a value $D$ so that it does not respond to price changes (i.e. the demand is \alert{inelastic}) up to a very high marginal utility $V >> o_s\,\, \forall s$, i.e.
  \begin{align*}
    U(d) = Vd
  \end{align*}
  for $d\leq D$.
...
\begin{frame}{Next simplest example: several generators, fixed demand}
  ...
  \centering
  \includegraphics[width=8cm]{supply-demand-high.pdf}
...
\begin{columns}[T]
  \begin{column}{5cm}
    ...
    \vspace{.5cm}
    \includegraphics[width=5cm]{european_bidding_zones.png}
  \end{column}
...
\begin{frame}
  ...
  \centering \includegraphics[width=12cm]{fbmc}
...
\begin{frame}[fragile]
  ...
  %single_node_storage_bids.ipynb
    \centering \includegraphics[width=9cm]{storage_bids.pdf}
